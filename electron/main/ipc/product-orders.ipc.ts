...
export function registerProductOrderHandlers() {
  ...
  ipcMain.handle(IPC.PRODUCT_ORDERS.GET_PENDING, () => {
    return db.prepare(`
      SELECT po.*, p.name AS product_name, p.barcode, p.retail_price, p.image_path
      FROM product_orders po
      JOIN products p ON p.id = po.product_id
  ...
  ipcMain.handle(IPC.PRODUCT_ORDERS.CREATE, (_e, data: any) => {
    const id = randomUUID()
    db.prepare(`
      INSERT INTO product_orders (id, product_id, vendor_name, quantity, unit_cost, expected_at, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(id, data.product_id, data.vendor_name || null, data.quantity, data.unit_cost || 0,
           data.expected_at || null, data.notes || null)
    return { id }
  })
